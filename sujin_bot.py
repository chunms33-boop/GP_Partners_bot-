...
async def idle_talker(bot: Bot):
    ...
    while True:
        ...
        # 취침 시간이면 패스
        is_sleep_time = (hour == 22 and minute >= 30) or hour == 23 or (0 <= hour < 9)
        if is_sleep_time:
            continue
        ...
        if idle_hourly_count >= 3:
            logger.info(f"[수진 idle] {hour}시 3번 완료 → 침묵")
            continue
        ...
        if silent_min < 60:
            logger.info(f"[수진 idle] 조용하지 않음 ({silent_min}분) → 패스")
            continue
        ...
        try:
            ...
            await save_log(0, "박수진", msg)  # DB 기록 → 다른 봇도 감지 가능
